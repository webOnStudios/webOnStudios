<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.0.0/dist/css/bootstrap.min.css" integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/css/bootstrap.min.css">
    <link rel="stylesheet" href="../../assets/CSS//TIENDA/privacidad-terminos.css">
    <link rel="icon" href="/imagenes/icono-blanco.ico" type="image/x-icon">
    <title>Acerca de</title>
</head>

<body>
    <header class="container-fluid" data-spy="affix" data-offset-top="197">

        <section class="row row justify-content-between align-items-center">
            <article class="col-md-1 mt-2 mt-md-0 col-3d-flex justify-content-start">
                
            </article>
            <article class="col-md-3 mt-2 mt-md-0 col-3 d-flex justify-content-start">
                <a href="index.html"><img src="data:image/png;base64," width="40"></a>
            </article>
            <article class="col-md-3 mt-2 mt-md-0 col-3 d-flex justify-content-center">
                <a href="index.html" class="nombre"><b>TIENDAS WEBON</b></a>
            </article>
            <article class="col-md-2 mt-2 mt-md-0 col-3 d-flex justify-content-end">
                <div class="menu">
                    <button><img src="data:image/png;base64,"> <span class="caret"></span></button>
                    <section class="menu-content">
                        <a href="login.html">Iniciar sesión ></a>
                        <a href="Register.html">Registrarse ></a>
                    </section>
                </div>
            </article>
            <article class="col-md-2 mt-2 mt-md-0 col-3 d-flex justify-content-center">
                <a href="carrito.html" class="carrito"><img src="data:image/png;base64,">Carrito</a>
            </article>
            <article class="col-md-1 mt-2 mt-md-0 col-3 d-flex justify-content-start">
                
            </article>
        </section>
    </header>
    <main class="container-fluid" id="barra-abajo">
        <section class="row justify-content-between align-items-center">
            <article class="col-md-2 mt-2 mt-md-0 col-3 d-flex justify-content-end">
                <a href="index.html" class="segunda-linea">Inicio</a>
            </article>
            <article class="col-md-2 mt-2 mt-md-0 col-3 d-flex justify-content-center">
                <div class="menu">
                    <button>Categorias <span class="caret"></span></button>
                    <section class="menu-content">    
                        <a href="#opcion1">Electrónica ></a>
                        <a href="#opcion2">Hogar y cocina ></a>
                        <a href="#opcion3">Moda y ropa ></a>
                        <a href="#opcion4">Juguetes y Juegos ></a>
                        <a href="#opcion5">Deportes ></a>
                        <a href="#opcion5">Salud y Belleza ></a>
                        <a href="#opcion7">Libros y medios ></a>
                        <a href="#opcion8">Automóviles ></a>
                        <a href="#opcion9">Mascotas ></a>
                        <a href="#opcion11">Alimentos y bebidas ></a>
                        <a href="#opcion12">Otros...</a>
                    </section>
                </div>
            </article>
            <article class="col-md-1 mt-2 mt-md-0 col-3 d-flex justify-content-center">
                <a href="#empresas" class="segunda-linea">Empresas</a>
            </article>
            <article class="col-md-2 mt-2 mt-md-0 col-3 d-flex justify-content-start">
                <a href="#acercade" class="segunda-linea">Acerca de </a>
            </article>
            <article class="col-md-1 d-flex justify-content-start">
                
            </article>
            <article class="col-md-3 mt-2 mt-md-0 col-12 d-flex justify-content-start">
                <div class="input-group">
                    <input type="text" class="form-control form-control-sm" placeholder="Buscar..." name="search">
                    <div class="input-group-btn">
                        <a href="/var/www/vista/TIENDA/busqueda.html" class="btn btn-default btn-sm" type="submit">
                            <img src="data:image/png;base64,"/>
                        </a>
                    </div>
                </div>
            </article>
            <article class="col-md-1 mt-2 d-flex justify-content-start">
                
            </article>
        </section>
    </main>
    <h1>Acerca de Nosotros</h1>
    <div id="terminos">
        <p>
Bienvenidos a Tiendas Webon <br>
<br>
En Tiendas Webon, creemos que la compra y venta de productos debería ser una experiencia sencilla y accesible para todos. Nuestra plataforma conecta a usuarios que desean adquirir productos de calidad con empresas que buscan expandir su mercado y aumentar sus ventas. Nos apasiona facilitar esta conexión y crear un ecosistema digital vibrante y eficiente.
<br>
<br>
Nuestra Historia
<br>
<br>
Tiendas Webon nació en 2024 con el objetivo de democratizar el comercio electrónico. Desde el principio, nos comprometimos a ofrecer un espacio donde tanto pequeños emprendedores como grandes empresas puedan exhibir sus productos y llegar a un público amplio. A lo largo de los años, hemos crecido y evolucionado, convirtiéndonos en una de las plataformas de comercio electrónico más confiables del sector.
<br>
<br>
¿Qué Hacemos?
<br>

En Tiendas Webon, ofrecemos un espacio integral para:
<br>

<ul>
<li>Compradores: Encuentra una amplia variedad de productos de diversas categorías, desde tecnología hasta moda, todo en un solo lugar.</li>
<li>Vendedores: Ofrecemos a las empresas una plataforma fácil de usar para listar sus productos, gestionar ventas y alcanzar nuevos clientes.</li>
</ul>
<br>
Nuestro Enfoque
<br>
<br>
Nos enfocamos en crear una experiencia de usuario excepcional. Desde la navegación intuitiva hasta el proceso de compra simplificado, cada aspecto de Tiendas Webon está diseñado pensando en nuestros usuarios. Creemos en la transparencia y en construir relaciones duraderas tanto con los compradores como con los vendedores.
<br>
<br>
Nuestro Equipo
<br>
<br>
El equipo de Tiendas Webon está compuesto por profesionales apasionados por el comercio electrónico, la tecnología y la atención al cliente. Trabajamos juntos para garantizar que nuestra plataforma sea segura, eficiente y siempre esté en evolución, adaptándose a las necesidades de nuestros usuarios.
<br>
<br>
¿Por Qué Elegir Tiendas Webon?
<br>
<ul></ul>
<li>Variedad: Una amplia gama de productos para satisfacer todas tus necesidades.</li>
<li>Facilidad de uso: Una plataforma intuitiva que simplifica la compra y venta de productos.</li>
<li>Soporte: Un equipo dedicado a ofrecer asistencia y resolver cualquier consulta que puedas tener.</li>
<li>Confianza: Comprometidos con la seguridad y la satisfacción del cliente, garantizando transacciones seguras.</li>
</ul>
<br>
Contáctanos
<br>
<br>
Si eres un comprador en busca de productos de calidad o una empresa que quiere expandir su presencia en línea, ¡Tiendas Webon es el lugar para ti! Contáctanos hoy mismo para comenzar tu viaje con nosotros y descubrir todas las oportunidades que ofrecemos.
</p>
</div>
    <footer class="container-fluid">
        <section class="row justify-content-center">
            <article class="col-md-3">
                <h5>Información útil</h5>
                <ul class="list-unstyled">
                    <li><a href="../../vista/TIENDA/acerca.html">¿Qué es TIENDAS WEBON?</a></li>
                    <li><a href="../../vista/TIENDA/index.html">Compra con WEBON</a></li>
                    <li><a href="../../vista/TIENDA/ayuda-usuario.html">Centro de ayuda en línea</a></li>
                    <li><a href="../../vista/TIENDA/terminos.html">Términos y Condiciones</a></li>
                </ul>
            </article>
            <article class="col-md-3">
                <h5>Todo en un solo lugar</h5>
                <ul class="list-unstyled">
                    <li>✔ Plataforma segura y confiable</li>
                    <li>✔ Gestión fácil de compras</li>
                    <li>✔ Análisis y reportes de compras detallados</li>
                    <li>✔ Soporte dedicado 24/7</li>
                </ul>
            </article>
            <article class="col-md-3">
                <h5>Compra con confianza</h5>
                <p>CEn nuestra plataforma, garantizamos una experiencia de compra segura y fiable. Cada transacción está respaldada por protocolos avanzados de seguridad, asegurando que tus datos y pagos estén siempre protegidos.</p>
    
            </article>
            <article class="col-md-3">
                <h5>Contacto</h5>
                <p>Atención telefónica: <a href="tel:+598 99 123 456">+598 99 123 456</a></p>
                <p>De Lunes a Sábados de 09:00hs a 18:00hs y Domingos de 10:00hs a 16:00hs</p>
                <ul class="list-unstyled">
                    <li><a href="#">Preguntas Frecuentes / Contacto</a></li>
                    <li><a href="../../vista/EMPRESAS/cuenta-empresa.html">Trabaja con nosotros</a></li>
                    <li><a href="#">Propiedad Intelectual</a></li>
                    <li><a href="#">Política de Calidad</a></li>
                </ul>
            </article>
        </section>
        <hr>
        <section class="row">
            <article class="col-md-6 text-end">
                <p>Desarrollado por:</p>
                <img src="../../assets/Imagenes/logoWebOn.png" alt="WebOn Studios Logo" height="90">
                <p>WebOn Studios</p>
            </article>
        </section>
</footer>
</body>
</html>